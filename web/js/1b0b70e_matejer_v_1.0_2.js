...
matejerApp
    ...
    .config(['$translateProvider', function($translateProvider) {
        ...
        .translations('fr', {
            'quick_view': 'Coup D\'œil',
            'subscribed': 'Abonné',
            'unsubscribed': 'S\'Abonner',
            'liked': 'Aimé',
            'unliked': 'Aime',
            'offer': 'Mettre En Vente',
            'error_title': 'Oups! Une erreur est survenue ...',
            'error_content': 'Une erreur interne s’est produite, veuillez actualiser la page et/ou réessayer.',
            'signFb': 'S\'inscrire Avec Facebook',
            'signGp': 'S\'inscrire Avec Google+',
            'help': 'Aide',
            'terms': 'termes et conditions',
            'new_list': 'Nouvelle Liste [valeurs]',
            'new_table': 'Nouvelle Liste [nom, valeur]',
            'double_name': 'Table',
            'simple_name': 'Liste',
            'value_ph': 'La valeur de la caractéristique',
            'name_ph': 'Le nom de la caractéristique',
            'group_ph': 'Le nom du groupe de caractéristiques',
            'add_color': 'Ajouter Couleur(s)',
            'your_review': 'Votre Avis',
            'edit_btn': 'Modifier',
            'remove_btn': 'Supprimer',
            'cancel_btn': 'Annuler',
            'remove_edit_m_title': 'Confirmation De Suppression',
            "item_rm_confirm": "Êtes-vous sûr que vous voulez supprimer {[{name}]}?",
            'this_item': 'cet objet',
            'cat_edit_lckd_tltp': 'Le mode de modification est verrouillé, cliquez pour déverrouiller',
            'cat_edit_ulckd_tltp': 'Le mode de modification est déverrouillé, cliquez pour verrouiller',
            'share_product': 'Partager ce produit',
            'abo_image_shop': 'Mettez à jour le logo / couverture de votre boutique',
            'message': 'Contacter'
        })
        .translations('ar', {});
...
matejerApp
    .service('APIService', ['$http', '$q', '$compile', function($http, $q, $compile){
        ...
        function injectData( response, element ) {
            $.each( response.injections, function(key, value){
                element.find(value.injectIn).html(value.content);
            });
        }
        function compileInjectData( response, element, scope ) {
            $.each(response.injections, function(key, value){
                var compiled = $compile(value.content)(scope);
                element.find(value.injectIn).html( compiled );
            });
        }
        function htmlToJson(htmlResp){
            var jsonResponse = {'stat': $(htmlResp).attr('data-stat'), 'injections': []};
            $.each($(htmlResp).find('.injection'), function() {
                jsonResponse.injections.push({'injectIn': '#'+$(this).attr('data-id'), 'content': $(this).html()});
            });
            return jsonResponse;
        }
        function toJson(htmlResp){
            var jsonResponse = {'stat': $(htmlResp).attr('data-stat')};
            typeof $(htmlResp).attr('data-title') == 'undefined' ? false : jsonResponse.title = $(htmlResp).attr('data-title');
            typeof $(htmlResp).attr('data-form-stat') == 'undefined' ? false : jsonResponse.form_stat = $(htmlResp).attr('data-form-stat');
            typeof $(htmlResp).attr('data-template') == 'undefined' ? false : jsonResponse.template = $(htmlResp).html();
            typeof $(htmlResp).attr('data-content') == 'undefined' ? false : jsonResponse.content = $(htmlResp).html();
            return jsonResponse;
        }
    ...
    .service('UtilService', ['$rootScope' ,'$uibModal', function($rootScope, $uibModal){
        "use strict";
        ...
        function fileStyle() {
            $('.input-file-cont input').each(function(){
                ...
                document.getElementById(inputId).addEventListener("change", function(event){handleFileSelect(event, $(this).attr('well'))}, false);
            });
        }
        ...
        function selectDestry(selector) {
            $(selector).select2('destroy');
        }
        function select(){
            $("select").each(function () {
                var cls = $(this).attr('class');
                if( typeof cls !== 'undefined' &&  cls.indexOf('image-picker') < 0 &&  cls.indexOf('__language') < 0)
                    $(this).select2();
            });
        ...
        function ckEditor(){
            /*
            CKEDITOR.replace( 'abo_mainbundle_feature_description', {
                language: 'fr',
                customConfig: '/bundles/ckeditor/config.js'
            });*/
            ...
            CKEDITOR.replace('abo_mainbundle_feature_descriptionFr', {
                contentsLangDirection : 'ltr',
                language: 'fr'
            });
            CKEDITOR.replace( 'abo_mainbundle_feature_descriptionAr', {
    ...
    .service('URLService', [function(){
        ...
        function messageUrl(slug){
            return '/app_dev.php/modal/message/'+slug;
        }
        function imageFormUrl(entity){
            return '/modal/image-'+entity;
        }
        ...
        function removeEditUrl(type, entity, action, id){
            ...
            switch(str){
                case 'shop-cat':
                    return '/myShop/categories/' + action + '/' + id;
                case 'shop-col':
                    return '/myShop/collections/' + action + '/' + id;
                case 'shop-pay':
                    return '/myShop/payments/' + action + '/' + id;
                case 'user-cat':
                    return '/myProfile/categories/' + action + '/' + id;
                default:
                    if (type === 'cat') {
                        return '/TM/' + entity + '/categories/' + action + '/' + id;
                    } if (type === 'arr') {
                        return '/TM/' + entity + '/categories/' + action + '/' + id;
                    }
            }
        ...
        function passwordUnlockUrl(type, slug){
            if(type === 'shop'){
                return '/myShop/password-unlock';
            }else {
                return '/TM/'+slug+'/password-unlock';
            }
        }
        ...
        function availibilityUrl(value){
            return '/api/is-available/username/'+value;
        }
        function subscribeUrl(entity, slug){
            return '/api/subscribe/'+entity+'/'+slug;
        }
        function likeUrl(id){
            return '/api/like/'+id;
        }
        function quickViewUrl(entity, slug){
            return '/modal/'+ locale +'/'+entity+'/'+slug;
        }
        function offerFormUrl(slug){
            return '/modal/offer-form/'+slug;
        }
        function colorsUrl(){
            return '/modal/'+ locale +'/colors';
        }
        function galleryUrl(tab, search, page) {
            return '/gallery-'+tab+'/'+page+'/'+search;
        }
        function helpUrl(group) {
            return '/modal/'+ locale +'/help/'+group;
        }
...
matejerApp
    ...
    .controller('EditProfileController', ['$scope', '$rootScope', '$element', '$compile', '$filter', 'APIService', 'UtilService', function($scope, $rootScope, $element, $compile, $filter, api, util){

        'use strict';
        $scope.address = {state: 0, country: 0};
        $scope.$on('country_val', function(event, val) { $scope.address.country = val; });
        $scope.$on('state_val', function(event, val) { $scope.address.state = val; });
        ...
        $scope.getForm = function(type){
            ...
            request . then(function successCallback(response) {
                    ...
                    if(type === 'address'){
                        ...
                        api.injectData( response , $element );
                    if(type === 'contact')
                        util.inputMask();
                }, function errorCallback(response) {
                    util.notify($filter('translate')('error_title'), $filter('translate')('error_content'), 'danger');
        ...
        $scope.submitForm =  function(type, event){
            ...
            request . then(function successCallback(response) {
                    ...
                    if(type === 'contact'){
                        ...
                        api.injectData( response , $element );
                    if( response.stat === "form_submit_success" )
...
matejerApp
    ...
    .directive('message', ['$compile', '$uibModal', '$filter', 'APIService', 'UtilService', 'URLService', function($compile, $uibModal, $filter, api, util, url) {
        'use strict';
        return {
            restrict: 'EA',
            replace: true,
            ...
            controller: function($scope, $element) {
                $scope.loading = false;
                $scope.msgText = $filter('translate')('message');
                var options = {controller: 'InstanceModalController'};
                $scope.getModal = function(){
                    $scope.loading = true;
                    var request = api.getData('GET', url.messageUrl($scope.shopSlug));
                    request . then(function successCallback(response) {
                        response = api.toJson(response);
                        if(response.stat === 'success'){
                            options.template = response.template;
                ...
                function submit(){
                    var data = angular.element('#form_shop_images').serialize();
                    var action = angular.element('#form_shop_images').attr('action');
                    var request = api.submitForm( action, data );
                    request . then(function successCallback(response) {
                        response = api.toJson(response);
                        if(response.form_stat === 'success'){
            ...
            template: '<button class="btn btn-{[{ theme }]} btn-sm card-btn" title="{[{ msgText }]}" ng-click="getModal()"><i class="fa fa-envelope-o" ng-hide="loading"></i><i class="fa fa-spinner fa-spin" ng-show="loading"></i> <span class="hidden-xs">{[{ msgText }]}</span></button>'
        };
    }])
    ...
    .directive('offer', ['$uibModal', '$filter', 'APIService', 'UtilService', 'URLService', function($uibModal, $filter, api, util, url) {
        ...
        return {
            ...
            template: '<button class="btn btn-{[{ theme }]} btn-sm card-btn" title="{[{ offerText }]}" ng-show="isOfferable" ng-click="offer()"><i class="fa fa-tag" ng-hide="loading"></i><i class="fa fa-spinner fa-spin" ng-show="loading"></i></button>'// <span class="hidden-xs">{[{ offerText }]}</span>
        };
    }])
...
;
$(document).ready(function(){$('.ftr-desc a').each(function () {$(this).attr('target', '_blank');});});
// Image Picker
(function() {
